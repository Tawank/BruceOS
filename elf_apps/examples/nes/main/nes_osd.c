 ...
 * steady clock, so audio is pulled once per video frame call.
 *
 ...
 * countdown decremented one sample at a time inside its own mixing loop,
 * not something that assumes a fixed call size -- apu.num_samples (set from
 * this same sample_rate / NES_REFRESH_RATE in apu_setparams()) only sizes
 * the countdown *tables* built once at startup, in units of samples, so it
 * stays correct however the samples arrive.
 *
...
static void audio_pump(void) {
     ...
     * UINT32_MAX), and the whole expression is 32-bit throughout -- the same
     * "no 64-bit divide helper available to ELF apps" constraint documented
     * on pace_frame() below applies equally here, and this avoids it the
     * same way: no operand is ever a 64-bit value. */
