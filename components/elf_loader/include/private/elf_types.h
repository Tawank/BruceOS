...
typedef struct esp_elf_sec {
    ...
    uintptr_t       addr;               /*!< final runtime address */
    uintptr_t       reloc_addr;         /*!< writable relocation address */
    size_t          size;               /*!< section size */
} esp_elf_sec_t;

typedef struct esp_elf_xip_ops {
    int (*allocate)(void *context, size_t size, const uint8_t **instruction,
                    const uint8_t **data, uint32_t *handle);
    int (*write)(void *context, uint32_t handle, size_t offset,
                 const void *data, size_t size);
    void (*release)(void *context, uint32_t handle);
} esp_elf_xip_ops_t;
...
typedef struct esp_elf {
    ...
    const esp_elf_xip_ops_t *xip_ops;
    void            *xip_context;
    uint32_t        xip_handle;
