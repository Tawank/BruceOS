...
/* BruceOS: undef'd -- see BRUCEOS_FORK.md. jdcoefct.c pads the resident
 * virtual-array window 5x ("access_rows *= 5") purely so block smoothing
 * could be turned on later; BruceOS always decodes with
 * do_block_smoothing = FALSE (core/image/jpg/jpg.c), so that headroom is
 * dead weight this target can't spare. */
