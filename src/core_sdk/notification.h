 ...
 * pushing process's own routed stdio session (BRUCE_STDIO_SESSION_INVALID if
 * it had none) into `*out_session`, and sets `*out_dismiss` to false; on a
 * dismiss, sets `*out_dismiss` to true and leaves the other four untouched.
 * Returns BRUCE_ERR_TIMEOUT if nothing arrived in time.
 *
 ...
 * pusher's), so nothing about rendering is special-cased in Core. A user who
 * wants a different notification UI does not need this function at all:
 * they can just call display__overlay_create() directly from their own app
 * instead of going through notification__push(). */
