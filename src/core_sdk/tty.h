...
/*
 * Terminal ("tty") metadata layered on top of a stdio session.
 *
...
/* True when the calling process's routed stdio session has a known terminal
 * size, i.e. some owner (a terminal app, the ssh client, ...) has called
...
/* Reads the calling process's routed session's terminal geometry. Returns
 * BRUCE_ERR_NOT_FOUND if no session is routed. */
bruce_result_t tty__get_size(bruce_tty_size_t *out_size);

/* Sets the terminal geometry of `session`. Only the session's owner (the
 * process that created it -- typically the terminal app or the ssh client)
 * may call this: it represents a physical resize, not a request from a
 * tenant process running inside the session. Returns BRUCE_ERR_PERMISSION
 * if the caller isn't the owner, BRUCE_ERR_NOT_FOUND if the session doesn't
 * exist. */
bruce_result_t tty__set_size(bruce_stdio_session_t session, uint16_t columns, uint16_t rows);

/* Cooperative bookkeeping only -- BruceOS never echoes or line-edits at the
 * session level, so switching modes changes no OS behavior by itself. This
 * exists so a process can declare its own intent (surfaced by `stty`, and
 * as the landing spot for any future tcsetattr()-shaped compatibility
 * shim) and so a nested reader can tell whether the terminal already
 * considers itself raw. Operates on the calling process's own routed
 * session; no ownership check, since it's just self-description. */
