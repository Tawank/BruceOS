...
/* The display__set_text_size() multiplier for a heading level (0 = body/link text). */
int browser_layout__heading_scale(int heading_level);

/* Word-wraps `doc` to `width` px using the given native (unscaled) font cell
 * size, calling `visitor` once per positioned word or image, in document
 * order. Returns the total laid-out content height in pixels. */
