...
static const char s_index_html[] =
    ...
    "place-items:center;background:#0a0d0b;color:#e8ffe9;font:16px/1.5 ui-monospace,monospace}"
    "main{width:min(42rem,calc(100% - 2rem));border:1px solid #55f27b;padding:clamp(1.5rem,5vw,3rem);"
    "box-shadow:8px 8px 0 #173d21}small{color:#80b98d}code{color:#55f27b}</style></head>"
    "<body><main><small>BRUCE IDF / WEB SERVICE</small><h1>WebUI is running.</h1>"
    "<p>The native server foundation is online. Administrative file, input, and command APIs are not enabled "
    "in this initial security-safe slice.</p><p>Status: <code id=\"status\">loading</code></p></main>"
    "<script>const out=document.getElementById('status');fetch('/api/status').then(r=>r.json())"
    ".then(s=>out.textContent=`${s.mode} / ${s.ip}:${s.port}`).catch(()=>out.textContent='unavailable')"
    "</script></body></html>";
...
static int webui_app__status(bool gui)
{
    ...
    if (status.running) {
        ...
        snprintf(message, sizeof(message), "Running on http://%s:%u\nNetwork: %s", ip != NULL ? ip : "unknown",
                 status.port, s_network_mode == WEBUI_APP_NETWORK_AP ? "access point" : "existing Wi-Fi");
    } else {
...
static int webui_app__start(webui_app_network_mode_t mode, bool gui)
{
    ...
    int status_len = snprintf(status_json, sizeof(status_json),
                              "{\"running\":true,\"mode\":\"%s\",\"ip\":\"%s\",\"port\":80}",
                              mode == WEBUI_APP_NETWORK_AP ? "ap" : "existing", ip != NULL ? ip : "");
...
int webui_app_main(int argc, char **argv)
{
    ...
    if (strcmp(argv[0], "start") == 0 && argc == 2) {
        if (strcmp(argv[1], "ap") == 0) return webui_app__start(WEBUI_APP_NETWORK_AP, false);
        if (strcmp(argv[1], "sta") == 0) return webui_app__start(WEBUI_APP_NETWORK_EXISTING, false);
    }
    webui_app__usage();
