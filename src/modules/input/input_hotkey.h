 ...
 *
 *   UP DOWN LEFT RIGHT SELECT BACK MENU HOME DELETE
 *   BTN_A BTN_B BTN_C BTN_X BTN_Y BTN_L1 BTN_R1 BTN_L2 BTN_R2
 *   BTN_START BTN_SELECT BTN_THUMB_L BTN_THUMB_R
 *
 * plus the friendly navigation aliases NAVIGATION_NEXT/PREV/SELECT/BACK,
 * which name the same codes as DOWN/UP/SELECT/BACK.
 *
 ...
 * instead -- e.g. "emit NAVIGATION_NEXT" makes a button behave like Down.
 */
 ...
 * "UP", "NAVIGATION_NEXT"). Returns false if the name is not recognized. */
 ...
 * BRUCE_INPUT_CODE_BUTTON_B -> "BTN_B"), or NULL if it has none. Never
 * returns a navigation alias. */
