...
bool selftest__run_apprunner_registration_case(void) {
    ...
    if (app_runner__register(
            "selftest", "Run hardware and Core self-tests", "Test", selftest__apprunner_dummy_entry, 0
        ) != BRUCE_ERR_ALREADY_EXISTS) {
        printf("[selftest] apprunner/registration: duplicate name was not rejected\n");
        return false;
    }
    if (app_runner__register(NULL, "Invalid command", "Test", selftest__apprunner_dummy_entry, 0) !=
        BRUCE_ERR_INVALID_ARGUMENT) {
        printf("[selftest] apprunner/registration: NULL name was accepted\n");
        return false;
    }
    if (app_runner__register("invalid_description", NULL, "Test", selftest__apprunner_dummy_entry, 0) !=
        BRUCE_ERR_INVALID_ARGUMENT) {
    ...
    for (size_t i = 0; i < sizeof(capacity_names) / sizeof(capacity_names[0]); ++i) {
        bruce_result_t result = app_runner__register(
            capacity_names[i], "AppRunner capacity probe", "Test", selftest__apprunner_dummy_entry, 0
        );
...
bool selftest__run_apprunner_args_case(void) {
    ...
    bruce_result_t registered = app_runner__register(
        "selftest_echo", "AppRunner argument probe", "Test", selftest__apprunner_echo_entry, 0
    );
