...
/* Deliberately Core-private includes: see the exemption documented in
 * selftest.h and migration_BruceIDF.md. */
...
static void selftest__resource_cleanup(void *context)
{
...
static bool selftest__run_task_normal_exit_case(void)
{
...
static bool selftest__run_task_killed_case(void)
{
    memset(&s_shared, 0, sizeof(s_shared));
...
/* Add more `selftest__run_*_case()` functions here as Core modules grow
 * (storage, dialog, wifi, ...) and call them from selftest_app() below. */
...
int selftest_app(int argc, char **argv)
{
    ...
    bool ok = selftest__run_task_normal_exit_case();
    ok = selftest__run_task_killed_case() && ok;
