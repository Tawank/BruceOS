...
static bruce_result_t selftest__storage_dialog_mock_provider(const char *title, const char *message,
...
static void selftest__storage_dialog_mock_clear(void)
{
...
static int selftest__storage_open_denied_entry(int argc, char **argv)
{
    ...
    s_open_result.result = storage__open("/selftest_denied.txt",
...
bool selftest__run_storage_permission_denied_case(void)
{
    ...
    bool ok = created && (wait_result == BRUCE_OK || wait_result == BRUCE_ERR_NOT_FOUND) && s_open_result.ran &&
              s_open_result.result == BRUCE_ERR_PERMISSION;
    printf("[selftest] storage/permission-denied: %s (result=%d)\n", ok ? "OK" : "FAIL", s_open_result.result);
    return ok;
}
...
bool selftest__run_storage_protected_path_case(void)
{
    ...
    printf("[selftest] storage/protected-path: %s (bruce.json=%d permissions.json=%d)\n", ok ? "OK" : "FAIL",
           bruce_json, permissions_json);
    return ok;
}
...
bool selftest__run_storage_roundtrip_case(void)
{
    ...
    bruce_result_t write_result =
        opened == BRUCE_OK ? storage__write(write_file, written_text, sizeof(written_text) - 1, &written) : opened;
    ...
    bruce_result_t read_result =
        reopened == BRUCE_OK ? storage__read(read_file, read_buffer, sizeof(read_buffer) - 1, &read_size) : reopened;

    uint64_t position = 0;
    bruce_result_t seek_result = reopened == BRUCE_OK ? storage__seek(read_file, 0, SEEK_SET, &position) : reopened;
    char reread_buffer[32] = {0};
    size_t reread_size = 0;
    bruce_result_t reread_result = reopened == BRUCE_OK
    ...
    printf("[selftest] storage/roundtrip: %s (write=%d read=%d text=\"%s\")\n", ok ? "OK" : "FAIL", write_result,
...
static int selftest__storage_owner_entry(int argc, char **argv)
{
    ...
    bruce_file_id_t file = BRUCE_FILE_ID_INVALID;
    bruce_result_t opened = storage__open(
        "/selftest_ownership.txt", BRUCE_STORAGE_OPEN_WRITE | BRUCE_STORAGE_OPEN_CREATE | BRUCE_STORAGE_OPEN_TRUNCATE,
        &file);
    s_owner_file_id = opened == BRUCE_OK ? file : BRUCE_FILE_ID_INVALID;
    s_owner_ready = true;
    while (!s_owner_should_exit) {
...
bool selftest__run_storage_ownership_case(void)
{
    ...
    for (int i = 0; i < 200 && !s_owner_ready; ++i) {
        vTaskDelay(pdMS_TO_TICKS(10));
    }
    ...
    printf("[selftest] storage/ownership: %s (owner_ready=%d access_result=%d)\n", ok ? "OK" : "FAIL", s_owner_ready,
...
static int selftest__storage_leak_open_entry(int argc, char **argv)
{
    ...
    bruce_result_t opened = storage__open(
        "/selftest_leak.txt", BRUCE_STORAGE_OPEN_WRITE | BRUCE_STORAGE_OPEN_CREATE | BRUCE_STORAGE_OPEN_TRUNCATE,
        &file);
...
bool selftest__run_storage_no_leak_normal_exit_case(void)
{
    ...
    printf("[selftest] storage/no-leak-normal-exit: %s (%d iterations without exhausting the open-file table)\n",
           ok ? "OK" : "FAIL", SELFTEST__STORAGE_LEAK_ITERATIONS);
    return ok;
}
...
static int selftest__storage_leak_block_entry(int argc, char **argv)
{
    ...
    (void)argv;
    bruce_file_id_t file = BRUCE_FILE_ID_INVALID;
    storage__open("/selftest_leak_killed.txt",
...
bool selftest__run_storage_no_leak_killed_case(void)
{
    ...
    printf("[selftest] storage/no-leak-killed: %s (killed=%d reopen_ok=%d)\n", ok ? "OK" : "FAIL", killed, reopen_ok);
    return ok;
}
