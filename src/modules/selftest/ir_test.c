...
bool selftest__run_ir_validation_case(void) {
    uint32_t timing = 560;
    uint32_t zero_timings[] = {0, 0};
    bruce_ir_code_t invalid_code = {.type = IR_CODE_RAW};
    bool ok = ir__transmit(NULL, "NEC", 32, 0) == BRUCE_ERR_INVALID_ARGUMENT &&
              ...
              ir__transmit_raw(zero_timings, 2, 0, 0) == BRUCE_ERR_INVALID_ARGUMENT &&
              ir__receive(false, 0, NULL, 0) == BRUCE_ERR_INVALID_ARGUMENT &&
              ir__transmit_code(NULL, 0) == BRUCE_ERR_INVALID_ARGUMENT &&
              ir__transmit_code(&invalid_code, 0) == BRUCE_ERR_INVALID_ARGUMENT &&
