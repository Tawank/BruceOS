...
/* Shell built-ins (`cd`, `kill`, ...) aren't app_runner commands -- they have
 * no process of their own for `man <name>` to launch with --help, so they get
 * their own short listing here instead of a section in the loop above. */
static void man_app__list_builtins(void) {
    stdio__printf("\nShell built-ins:\n");
    for (size_t i = 0; i < shell_builtins__count(); ++i) {
        const char *name = shell_builtins__name(i);
        if (name == NULL) continue;
        const char *description = shell_builtins__description(i);
        stdio__printf("%s - %s\n", name, description != NULL ? description : "");
    }
}
...
/* Shell built-ins get their own TOC + body section, appended after every
 * app_runner category (see man_app__gen_md() below) -- there's no separate
 * process to run "--help" on (that's the whole point of a builtin, see
 * shell_jobs__kill()'s own doc comment), so the body entry is just the name
 * and description, no captured --help code block. */
static void man_app__gen_md_builtins_toc(man_app__buf_t *buf) {
    if (shell_builtins__count() == 0) return;
...
static void man_app__gen_md_builtins_body(man_app__buf_t *buf) {
    if (shell_builtins__count() == 0) return;
    man_app__buf_printf(buf, "\n## Shell Built-ins\n");
    for (size_t i = 0; i < shell_builtins__count(); ++i) {
        const char *name = shell_builtins__name(i);
        ...
        if (description != NULL && description[0] != '\0') man_app__buf_printf(buf, "%s\n", description);
    }
}
...
static int man_app__gen_md(bool use_marker) {
    ...
    man_app__gen_md_builtins_toc(&buf);
    ...
    man_app__gen_md_builtins_body(&buf);
...
/* `man <builtin>` (e.g. "man kill"): a builtin has no process of its own for
 * man_app__page()/man_app__show_command() to run --help on, so it's handled
 * separately, straight from man_app_main() -- just the same name and
 * description man_app__gen_md_builtins_body() puts in COMMANDS.md, printed
 * directly rather than paged through `less` (there's nowhere near enough
 * text here to need it). */
static int man_app__describe_builtin(const char *name) {
    for (size_t i = 0; i < shell_builtins__count(); ++i) {
        const char *builtin_name = shell_builtins__name(i);
        if (builtin_name == NULL || strcmp(builtin_name, name) != 0) continue;
        const char *description = shell_builtins__description(i);
        stdio__printf(
            "%s - %s\n\n%s is a shell built-in, not a separate command -- see \"shell help\" for the "
            "full built-in list.\n",
...
int man_app_main(int argc, char **argv) {
    ...
    else if (command != NULL && man_app__find_command(command) == NULL && shell_builtins__is_builtin(command))
        result = man_app__describe_builtin(command);
