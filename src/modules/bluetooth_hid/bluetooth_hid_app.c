...
static bool bluetooth_hid_app__parse_address(const char *text, uint8_t address[BRUCE_BLUETOOTH_ADDRESS_LEN])
{
    ...
    int consumed = 0;
    if (text == NULL || sscanf(text, "%2x:%2x:%2x:%2x:%2x:%2x%n", &bytes[0], &bytes[1], &bytes[2],
                               &bytes[3], &bytes[4], &bytes[5], &consumed) != BRUCE_BLUETOOTH_ADDRESS_LEN ||
        text[consumed] != '\0') {
        return false;
    }
...
static int bluetooth_hid_app__connect(const uint8_t address[BRUCE_BLUETOOTH_ADDRESS_LEN], bool gui)
{
    ...
    if (gui) {
        (void)dialog__message(BRUCE_DIALOG_SUCCESS, "Bluetooth HID",
...
static int bluetooth_hid_app__scan_terminal(void)
{
    ...
    for (int i = 0; i < count; ++i) {
        printf("%02X:%02X:%02X:%02X:%02X:%02X rssi=%d type=%s name=%s\n", devices[i].address[0],
               devices[i].address[1], devices[i].address[2], devices[i].address[3], devices[i].address[4],
               devices[i].address[5], devices[i].rssi, bluetooth_hid_app__usage_name(devices[i].usage),
...
static int bluetooth_hid_app__scan_and_connect_gui(void)
{
    ...
    for (int i = 0; i < count; ++i) {
        snprintf(labels[i], sizeof(labels[i]), "%s (%s, %d dBm)",
                 devices[i].name[0] != '\0' ? devices[i].name : "(unnamed)",
                 bluetooth_hid_app__usage_name(devices[i].usage), devices[i].rssi);
...
static int bluetooth_hid_app__gui(void)
{
    if (!bluetooth_hid__is_supported()) {
        (void)dialog__message(BRUCE_DIALOG_WARNING, "Bluetooth HID",
                              "Classic Bluetooth is unavailable on this target.");
