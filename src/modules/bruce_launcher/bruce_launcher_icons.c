...
static void bruce_launcher__draw_thick_line(int x0, int y0, int x1, int y1,
...
static void bruce_launcher__draw_bluetooth_rune(int cx, int cy, int size, int stroke, uint16_t color)
{
    ...
    bruce_launcher__draw_thick_line(cx + arm, cy - arm / 2, cx - arm / 2, cy + arm / 2,
                                    stroke, color);
    bruce_launcher__draw_thick_line(cx - arm / 2, cy - arm / 2, cx + arm, cy + arm / 2,
                                    stroke, color);
...
void bruce_launcher__draw_entry_icon(const bruce_launcher_entry_t *entry, int cx, int cy,
                                      int size, uint16_t color)
{
    ...
    switch (icon) {
        ...
        case BRUCE_LAUNCHER_ICON_WEBUI: {
            ...
            bruce_launcher__draw_thick_line(cx - radius * 4 / 5, cy - radius / 2,
                                            cx + radius * 4 / 5, cy - radius / 2, stroke, color);
            bruce_launcher__draw_thick_line(cx - radius * 4 / 5, cy + radius / 2,
                                            cx + radius * 4 / 5, cy + radius / 2, stroke, color);
        ...
        case BRUCE_LAUNCHER_ICON_BLUETOOTH:
            ...
            bruce_launcher__draw_arc_band(cx - size / 12, cy, size * 7 / 16, size * 3 / 8,
                                          220, 320, color);
            break;
        case BRUCE_LAUNCHER_ICON_BLUETOOTH_HID: {
            ...
            display__draw_round_rect(cx - size * 3 / 8, keyboard_y, size * 3 / 4,
                                     size / 3, size / 16, color);
            for (int row = 0; row < 2; ++row) {
                bruce_launcher__draw_thick_line(cx - size / 4, keyboard_y + size / 10 + row * size / 10,
                                                cx + size / 4, keyboard_y + size / 10 + row * size / 10,
                                                stroke, color);
            }
            break;
        }
        case BRUCE_LAUNCHER_ICON_IR: {
            ...
            bruce_launcher__draw_box(emitter_x - size / 4, cy - size * 3 / 8,
                                     size / 8, size * 3 / 4, stroke, color);
            bruce_launcher__draw_box(emitter_x - size / 8, cy - size / 4,
                                     size / 8, size / 2, stroke, color);
            display__fill_circle(emitter_x, cy, size * 7 / 64, color);
            for (int radius = size * 17 / 64; radius <= size * 37 / 64; radius += size * 10 / 64) {
                bruce_launcher__draw_arc_band(emitter_x, cy, radius + stroke - 1, radius,
                                              220, 320, color);
            }
            break;
        }
        case BRUCE_LAUNCHER_ICON_NRF24: {
            ...
            display__draw_round_rect(board_x + board_w, cy - board_h / 3,
                                     size / 5, board_h * 2 / 3, size / 12, color);
            bruce_launcher__draw_box(cx - size / 10, cy - size / 8,
                                     size / 5, size / 4, stroke, color);
            int pin_r = size >= 48 ? 2 : 1;
            for (int row = -1; row <= 1; row += 2) {
                for (int col = 0; col < 4; ++col) {
                    display__fill_circle(board_x + size / 10 + col * size / 8,
                                         cy + row * board_h / 3, pin_r, color);
                }
            }
            break;
        }
        case BRUCE_LAUNCHER_ICON_FILES: {
            ...
            for (int page = 1; page >= -1; --page) {
                bruce_launcher__draw_box(cx - page_w / 2 + page * step,
                                         cy - page_h / 2 - page * step,
                                         page_w, page_h, stroke, color);
            }
            break;
        }
        case BRUCE_LAUNCHER_ICON_TERMINAL: {
            display__draw_round_rect(left + pad, top + size / 6, size - 2 * pad,
                                     size * 2 / 3, size / 12, color);
            int prompt_y = cy - size / 12;
            bruce_launcher__draw_thick_line(cx - size / 4, prompt_y - size / 10,
                                            cx - size / 8, prompt_y, stroke, color);
            bruce_launcher__draw_thick_line(cx - size / 8, prompt_y,
                                            cx - size / 4, prompt_y + size / 10, stroke, color);
            bruce_launcher__draw_thick_line(cx, prompt_y + size / 10,
                                            cx + size / 4, prompt_y + size / 10, stroke, color);
            break;
        }
        ...
        case BRUCE_LAUNCHER_ICON_NOTIFICATION: {
            ...
            bruce_launcher__draw_thick_line(cx - radius, bell_y, cx - radius,
                                            bell_y + size / 5, stroke, color);
            bruce_launcher__draw_thick_line(cx + radius, bell_y, cx + radius,
                                            bell_y + size / 5, stroke, color);
            bruce_launcher__draw_thick_line(cx - radius - stroke, bell_y + size / 5,
                                            cx + radius + stroke, bell_y + size / 5, stroke, color);
            display__fill_circle(cx, bell_y + size / 5 + size / 12,
                                 size >= 48 ? 3 : 2, color);
            break;
        }
        ...
        case BRUCE_LAUNCHER_ICON_COMMAND:
            display__draw_round_rect(left + pad, top + pad, size - 2 * pad,
                                     size - 2 * pad, size / 10, color);
