...
JSValue js_load(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv)
{
    ...
    if (argc < 1 || !JS_IsString(ctx, argv[0])) {
        return JS_ThrowTypeError(ctx, "load(path:string)");
    }
    ...
    if (open_result != BRUCE_OK) {
        return JS_ThrowReferenceError(ctx, "load: cannot open %s", path);
    }
    ...
    if (storage__seek(file, 0, SEEK_END, &size) != BRUCE_OK || size == 0 || size > JS_STDLIB_LOAD_MAX_SIZE) {
        ...
        if (source == NULL) {
            ...
            if (storage__seek(file, 0, SEEK_SET, NULL) != BRUCE_OK) {
                ...
                size_t read_size = 0;
                if (storage__read(file, source, (size_t)size, &read_size) != BRUCE_OK || read_size != (size_t)size) {
                    result = BRUCE_ERR_IO;
                } else {
                    source[size] = '\0';
