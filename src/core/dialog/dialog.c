 ...
 * title bar and body message at -- bigger than the rest of the chrome
 * (DIALOG__TEXT_SIZE) since a message dialog is nothing but this text, so it
 * reads as the main content rather than another status line. */
#define DIALOG__MESSAGE_TEXT_SIZE 2

/* Shared frame for dialog__gui_message()/dialog__gui_message_show(): a
 * bordered popup window, same shape as a windowed dialog__choice() (compare
 * the render_window path above), rather than the full-bleed screen every
 * other dialog__gui_* draws. Its title bar is filled with `kind`'s own
...
static bruce_result_t dialog__gui_message_frame(bruce_dialog_kind_t kind, const char *title, const char *message) {
    uint16_t pri, sec, bg, surface, text, text_muted, border, success, warning, error;
    dialog__get_colors(&pri, &sec, &bg, &surface, &text, &text_muted, &border, &success, &warning, &error);
    (void)sec;
    (void)text_muted;
...
/* Same layout as dialog__gui_message() above, minus the trailing
 * dialog__gui_wait_for_any_key() -- this draws once and returns immediately.
 * Meant to sit on screen for whatever blocking call the caller makes right
 * after it, until that caller's own next dialog__* draws something else
 * full-screen over it. */
...
/* dialog_input.c owns the on-screen keyboard implementation. */
