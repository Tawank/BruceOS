...
static char display__latin_base(uint32_t c, display__accent_t *out_accent) {
    *out_accent = DISPLAY__ACCENT_NONE;
    switch (c) {
        ...
        case 0x010E: case 0x010F: *out_accent = DISPLAY__ACCENT_CARON; return c == 0x010E ? 'D' : 'd';
        ...
        case 0x0164: case 0x0165: *out_accent = DISPLAY__ACCENT_CARON; return c == 0x0164 ? 'T' : 't';
        ...
        case 0x017B: case 0x017C: *out_accent = DISPLAY__ACCENT_ACUTE; return c == 0x017B ? 'Z' : 'z';
        ...
        case 0x013D: case 0x013E: *out_accent = DISPLAY__ACCENT_CARON; return c == 0x013D ? 'L' : 'l';
...
static void display__draw_accent(display__process_context_t *context, int16_t x, int16_t y, display__accent_t accent) {
    ...
    if (accent == DISPLAY__ACCENT_CEDILLA) { display_internal__fill_rect(context, x + 2 * s, y + 7 * s, s, s, context->text_color); return; }
    if (accent == DISPLAY__ACCENT_STROKE) { display_internal__fill_rect(context, x, y + 3 * s, 5 * s, s, context->text_color); return; }
    if (accent == DISPLAY__ACCENT_NONE) return;
    display_internal__fill_rect(context, x + 2 * s, y, s, s, context->text_color);
    if (accent == DISPLAY__ACCENT_DIAERESIS || accent == DISPLAY__ACCENT_DOUBLE_ACUTE) {
    ...
    } else if (accent == DISPLAY__ACCENT_TILDE || accent == DISPLAY__ACCENT_CARON || accent == DISPLAY__ACCENT_BREVE) {
...
static void display__draw_char(display__process_context_t *context, int16_t x, int16_t y, uint32_t codepoint) {
    ...
    const uint8_t *glyph = display_internal__font_glyph(base);
