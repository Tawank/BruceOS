...
typedef struct {
    ...
    /* 0 selects Core-managed priority: tskIDLE_PRIORITY + 2 while foreground,
     * tskIDLE_PRIORITY + 1 while background. Set explicitly only for
     * Core-owned worker pools (e.g. http_server's async workers) that need to
     * keep a specific FreeRTOS priority across process state changes. */
    uint32_t priority;
