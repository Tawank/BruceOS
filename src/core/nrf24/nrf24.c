...
static void nrf24__ensure_mutex(void) {
    ...
    if (s_nrf24_mutex == NULL) { s_nrf24_mutex = xSemaphoreCreateMutexStatic(&s_nrf24_mutex_storage); }
...
bruce_result_t nrf24__init(void) {
    ...
    bruce_result_t result =
        gpio__configure_internal(CONFIG_BRUCE_NRF24_CE_GPIO, BRUCE_GPIO_MODE_OUTPUT, BRUCE_GPIO_PULL_NONE);
...
static bruce_result_t nrf24__transfer(
    uint8_t command, const uint8_t *write_data, uint8_t *read_data, size_t size, uint8_t *out_status
) {
    uint8_t tx[33] = {0};
    uint8_t rx[33] = {0};
...
nrf24__scan(uint8_t first_channel, size_t channel_count, uint8_t samples, uint8_t *out_activity) {
    ...
    if (result == BRUCE_OK) {
        result = nrf24__transfer(
            NRF24_CMD_W_REGISTER | NRF24_REG_RX_ADDR_P0, address, NULL, sizeof(address), NULL
        );
    }
