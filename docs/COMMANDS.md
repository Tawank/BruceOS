...
- [`tone`](#tone) - Play a square-wave audio tone
- [`top`](#top) - Processes, CPU, RAM usage
- [`wev`](#wev) - Print raw input events
...
- [`find`](#find) - Recursively search a directory tree for matching entries
...
- [`sftp`](#sftp) - Browse/download files over SFTP
- [`nc`](#nc) - TCP/UDP client, listener, and port scanner (netcat-style)
- [`nmap`](#nmap) - Host discovery and TCP/UDP port scanner (nmap-style)
...
- [`sed`](#sed) - Edit text with a stream editor script
...
- [`export`](#export) - Mark a variable for export to child processes
- [`clear`](#clear) - Erase the screen
- [`reset`](#reset) - Reset the terminal
- [`help`](#help) - List shell built-ins and usage
- [`exit`](#exit) - Exit the shell
- [`test`](#test) - Evaluate a conditional expression
- [`[`](#[) - Evaluate a conditional expression
- [`[[`](#[[) - Evaluate a conditional expression
- [`break`](#break) - Break out of a for/while loop
- [`continue`](#continue) - Skip to the next iteration of a for/while loop
- [`return`](#return) - Return from a shell function
- [`shift`](#shift) - Shift positional parameters left
- [`read`](#read) - Read a line into a variable
- [`time`](#time) - Time how long a command takes
- [`local`](#local) - Declare a function-local variable
- [`jobs`](#jobs) - List background jobs
- [`wait`](#wait) - Wait for a background job to finish
- [`kill`](#kill) - Send a signal to a job or process
- [`fg`](#fg) - Bring a background job to the foreground
- [`bg`](#bg) - Resume a paused job in the background
- [`disown`](#disown) - Stop tracking a job without touching it
- [`trap`](#trap) - Run a command when a signal is received or the shell exits
...
Play a square-wave audio tone

```
Usage: tone [frequency] [duration]

Play a square-wave tone through the board speaker or buzzer.

Arguments:
    frequency
              Frequency in Hz (default: 500)
...
Print raw input events

```
Usage: wev

Print raw input events. Press Back or Ctrl+C to stop.
...
Recursively search a directory tree for matching entries
...
Arguments:
    path      Directory to search (defaults to the working directory)

Options:
    -n, --name <value>
              Only match entries whose name fits this glob pattern (*, ?)

    -i        Match -name case-insensitively

    -t, --type <value>
              Only match this type: f (file) or d (directory)

    -m, --maxdepth <value>
              Descend at most this many levels below the start path
...
TCP/UDP client, listener, and port scanner (netcat-style)
...
Host discovery and TCP/UDP port scanner (nmap-style)
...
Arguments:
    host (required)
              Hostname or IPv4 address to ping
...
Options:
    ...
    -W <value>
              Time to wait for each reply, in milliseconds
...
Edit text with a stream editor script

```
Usage: sed <script> [args...] [options]

Stream editor: applies one editing command to each line of files or stdin.

Arguments:
    script (required)
              s<delim>PATTERN<delim>REPLACEMENT<delim>[gi], [/PATTERN/]d, or [/PATTERN/]p

Options:
    -n        Suppress automatic printing; only an explicit 'p' command emits a line
...
List shell built-ins and usage
...
Break out of a for/while loop

### continue

**Category:** Shell Built-in

Skip to the next iteration of a for/while loop
...
Stop tracking a job without touching it

### trap

**Category:** Shell Built-in

Run a command when a signal is received or the shell exits
