...
- [`nc`](#nc) - Connect to or listen on a TCP socket
...
Connect to or listen on a TCP socket

```
Usage: nc [args...] [options]

netcat-style TCP client and listener. Connect with `nc <host> <port>` or listen with `nc -l <port>`. During a session, stdin is sent to the peer and received data is printed. Press Ctrl+] or Ctrl+D to close.
