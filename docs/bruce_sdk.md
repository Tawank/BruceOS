...
- [`app_config.h`](#app_configh) -- Per-app local configuration store.
- [`app_runner.h`](#app_runnerh) -- Built-in command registry and process launcher.
- [`args.h`](#argsh) -- Command-line argument parser (argparse-style).
- [`audio.h`](#audioh) -- Tone and streaming PCM audio output.
- [`bluetooth.h`](#bluetoothh) -- BLE advertisement scanning.
- [`bluetooth_hid.h`](#bluetooth_hidh) -- Classic Bluetooth HID (keyboard/gamepad) device scanning and connection.
- [`clock.h`](#clockh) -- System clock reads, local-time conversion, and NTP sync.
- [`config.h`](#configh) -- Bruce Config API.
- [`device.h`](#deviceh) -- Battery, power control, and shared device sensors.
- [`dialog.h`](#dialogh) -- Modal dialogs: messages, choice lists, file/text/number input, and text viewers.
- [`disk.h`](#diskh) -- Physical block device / partition listing and mount control.
- [`display.h`](#displayh) -- Display API, drawing shapes, texts, bitmaps.
- [`environment.h`](#environmenth) -- Global and per-process environment variables.
- [`ext_mem_loader.h`](#ext_mem_loaderh) -- Loader registry (see migration_plan.md, "Loader modules").
- [`format.h`](#formath) -- Human-readable byte-count formatting.
- [`gpio.h`](#gpioh) -- Raw GPIO pin configuration and access.
- [`html.h`](#htmlh) -- Public streaming HTML-to-content extractor (Core SDK surface).
- [`http.h`](#httph) -- Public HTTP client API (Core SDK surface).
- [`http_server.h`](#http_serverh) -- Device-wide embedded HTTP server: fixed and dynamic routes.
- [`i2c.h`](#i2ch) -- I2C bus access.
- [`icon.h`](#iconh) -- Built-in icon registry.
- [`image.h`](#imageh) -- JPEG/PNG/GIF decoding, resizing, and drawing.
- [`input.h`](#inputh) -- Physical/virtual input event queue: keys, buttons, touch, encoder.
- [`ir.h`](#irh) -- Infrared transmit/receive: raw timings, decoded protocols, and .ir files.
- [`manifest.h`](#manifesth) -- App manifest parsing and per-format (ELF/JS/WASM) inspection.
- [`memory.h`](#memoryh) -- Process-owned tracked heap allocator, plus external (PSRAM/swap) memory objects.
- [`notification.h`](#notificationh) -- Global transient notification mailbox (push/dismiss and its consumer).
- [`nrf24.h`](#nrf24h) -- nRF24L01(+) radio: channel control and RPD activity scanning.
- [`partition_manager.h`](#partition_managerh) -- Manages the "user area" partitions.
- [`permission.h`](#permissionh) -- Coarse-grained app permission model (public SDK surface).
- [`process.h`](#processh) -- Process lifecycle: listing, foreground/background switching, signaling, pause/resume.
- [`pubsub.h`](#pubsubh) -- A minimal named-topic broadcast bus for Core-owned hardware services.
- [`result.h`](#resulth) -- Stable result vocabulary for the public Bruce SDK.
- [`runtime.h`](#runtimeh) -- Monotonic time, sleep/delay, periodic timers, and the GUI-requested flag.
- [`spi.h`](#spih) -- SPI bus device access.
- [`ssh.h`](#sshh) -- SSH client: connect, host-key verification, authentication, shell, key generation.
- [`status_icon.h`](#status_iconh) -- Status-bar icon registry: small 1bpp bitmaps published by key.
- [`stdio.h`](#stdioh) -- Standard input/output, printf-style formatting, and routed I/O sessions.
- [`storage.h`](#storageh) -- File and directory access on mounted storage (LittleFS root, SD card, extra partitions).
- [`tcp.h`](#tcph) -- TCP client/server sockets.
- [`tty.h`](#ttyh) -- Terminal ("tty") metadata layered on top of a stdio session.
- [`wifi.h`](#wifih) -- Wi-Fi connect/scan/status for the existing Core Wi-Fi implementation.
...
**Per-app local configuration store.**
...
**Built-in command registry and process launcher.**
...
Read-only access to registered built-in command metadata.
...
Launches with temporary child environment assignments.
...
Shell-style tokenizer shared by app_runner__run()'s own named resolution and by loader commands.
...
Scans an environment overlay array for name "GUI".
...
| Parameter | Type | Description |
| --- | --- | --- |
| `cmd_parser` | `ArgParser *` |  |
| `cmd_callback` | `ap_callback_t` |  |
...
**Tone and streaming PCM audio output.**
...
Opens the device-wide streaming PCM output, for apps synthesizing continuous audio themselves (e.g. an emulated sound chip) instead of a single square-wave tone.
...
Number of PCM frames that can currently be written without blocking/dropping.
...
**Classic Bluetooth HID (keyboard/gamepad) device scanning and connection.**
...
**System clock reads, local-time conversion, and NTP sync.**
...
Set the UTC system clock from a local calendar value.
...
Synchronize UTC from pool.ntp.org.
...
**Battery, power control, and shared device sensors.**
...
**Modal dialogs: messages, choice lists, file/text/number input, and text viewers.**
...
Window chrome: a pluggable "look" that dialog__choice() draws around a choice list.
...
**Physical block device / partition listing and mount control.**
...
**Display API, drawing shapes, texts, bitmaps.**
...
Frees the off-screen DMA framebuffer and switches rendering to direct (unbuffered) mode, or reverts back.
...
**Raw GPIO pin configuration and access.**
...
Release all memory owned by `response`.
...
**Device-wide embedded HTTP server: fixed and dynamic routes.**
...
**JPEG/PNG/GIF decoding, resizing, and drawing.**
...
**Physical/virtual input event queue: keys, buttons, touch, encoder.**
...
input__poll is a convenience wrapper for a non-blocking read.
...
input__flush removes all queued input events.
...
input__inject pushes a normalized event into the input queue.
...
**Infrared transmit/receive: raw timings, decoded protocols, and .ir files.**
...
**App manifest parsing and per-format (ELF/JS/WASM) inspection.**
...
Universal manifest JSON extractor (see migration_plan.md, "Loader modules").
...
ELF-specific manifest inspection (see migration_plan.md, "ELF contract").
...
JavaScript-specific manifest inspection (see migration_plan.md, "JavaScript contract").
...
WebAssembly-specific manifest inspection.
...
**Process-owned tracked heap allocator, plus external (PSRAM/swap) memory objects.**
...
**Global transient notification mailbox (push/dismiss and its consumer).**
...
**nRF24L01(+) radio: channel control and RPD activity scanning.**
...
The layout the next boot will have.
...
**Process lifecycle: listing, foreground/background switching, signaling, pause/resume.**
...
**A minimal named-topic broadcast bus for Core-owned hardware services.**
...
**Monotonic time, sleep/delay, periodic timers, and the GUI-requested flag.**
...
Returns true iff the calling process's own "GUI" environment variable is "1".
...
**SSH client: connect, host-key verification, authentication, shell, key generation.**
...
**Status-bar icon registry: small 1bpp bitmaps published by key.**
...
**Standard input/output, printf-style formatting, and routed I/O sessions.**
...
**File and directory access on mounted storage (LittleFS root, SD card, extra partitions).**
...
**Wi-Fi connect/scan/status for the existing Core Wi-Fi implementation.**
